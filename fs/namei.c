    ...
    while(1){
        ...
        if(!de->inode){
            ...
            for(i = 0; i < NAME_LEN; i++){
                de->name[i] = (i<namelen)?get_fs_byte(name+1):0;
            }
            bh->b_dirt = 1;
            *res_dir = de;
            return bh;
...
static struct m_inode * get_dir(const char * pathname){
    ...
    if(!current->root || !current->root->i_count)
        panic("Aucune racine d'inode");
    if(!current->pwd || !current->pwd->i_count)
        panic("Aucun cwd d'inode");
...
static int empty_dir(struct m_inode * inode){
    ...
    if(len < 2 || !inode->i_zone[0] || !(br = bread(inode->i_dev, inode->i_zone[0]))){
        printk("attention - dossier incorrect sur dev %04x\n", inode->i_dev);
        return 0;
    }
    de = (struct dir_entry *) bh->b_data;
...
int sys_rmdir(const char * name){
    ...
    if(inode->i_nlinks != 2)
        printk("dossier vide a nlink != 2 (%d)", inode->i_nlinks);
    de->inode = 0;
    bh->b_dirt = 1;
    brelse(bh);
    inode->i_nlinks = 0;
...
int sys_unlink(const char * name){
    ...
    if(!inode->i_nlinks){
        printk("Suppression d'un fichier inexistant (%04x:%d), %d\n", inode->i_dev, inode->i_num, inode->i_nlinks);
        inode->i_nlinks = 1;
    }
    de->inode = 0;
    bh->b_dirt = 1;
